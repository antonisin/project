    ...
    events:
      ...
      'click #delete'   : (e) -> 
        ...
        if @model.get 'isSystem'
          App.execute 'notify:error', App.request('lang:get', 'settings:delete:system')
        else
          @trigger 'click:delete', @model

  class List.ListEmptyView extends Air.BaseItemView
    template          : "admin/settings/empty"
    tagName           : "tr"

  class List.ListView extends Air.BaseCompositeView
    template              : 'admin/settings/list'
